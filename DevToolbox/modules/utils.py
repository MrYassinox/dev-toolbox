...
# LIB => matplotlib libraryies
from matplotlib import pyplot as plt

# LIB => skimage libraryies
from skimage import io, exposure

# LIB => sklearn libraryies
...
def wget(url: str, outputdir: str, logs: Optional[LoggerHandle]=None) -> None:
    ...
    try:
        ...
        with urlopen(url) as source, open(os.path.join(outputdir, basename), 'wb') as output:
            # DESC => Create a tqdm progress bar with the total file size
        ...
        if response.status_code == 200:
            ...
            if outputdir is not None:
                ...
                with open(os.path.join(outputdir, basename), 'wb') as f:
                    with tqdm(total=int(response.headers.get('Content-Length')), ncols=80, unit='iB', unit_scale=True) as loop:
...
# NOTE => Store metadata in a PNG image.
@logger.catch()
def store_metadata_png(data_dict: dict, img_path: str = '', overwrite: bool = False, return_info: bool = False):
    """Store metadata in a PNG image.

    Args:
        `data_dict` (dict): The dictionary containing the metadata.
        `img_path` (str, optional): The path to the PNG image. Defaults to ''.
        `overwrite` (bool, optional): Whether to overwrite the original image. Defaults to False.
        `return_info` (bool, optional): Whether to return the metadata information. Defaults to False.

    Returns:
        str: The metadata information, if return_info is True.

    Raises:
        Exception: If there is an error while storing the metadata.
    ...
    """
    try:
        # DESC => Split the filename and extension
        imgname, extension = os.path.splitext(os.path.basename(str(img_path)))
        ...
        # DESC => Open the image file with Pillow
        img = Image.open(img_path)
        metadata = PngInfo()

        # DESC => Open the image file with Pillow
        for key, value in data_dict.items():
            # DESC => Convert non-string values to strings
        ...
        # DESC => Save the new PNG image file with metadata
        if overwrite:
            filename = str(os.path.join(split_location, str(imgname + extension)))
        else:
            filename = str(os.path.join(split_location, str(imgname + '_metainfo' + extension)))
...
# NOTE => Store images in binary PIL mode.
@logger.catch()
def store_img_binary(files: Union[str, List[str]]) -> List[Image.Image]:
    """Store images in binary PIL mode.

    Args:
        `files` (Union[str, List[str]]): The file path or a list of file paths.

    Returns:
        List[Image.Image]: A list of PIL image objects in binary mode.

    Raises:
        Exception: If there is an error loading or reading the image file(s).
    ...
    ```
    Notes:
        - This function reads image files and stores them as PIL image objects in binary mode.
        - It accepts either a single file path as a string or a list of file paths.
        - The function uses the `open` function from PIL to open the image file(s) in binary mode.
        - Each loaded image is copied and added to the `IMAGES_PIL` list.
        - The resulting list contains PIL image objects that can be further processed or displayed.
    """
    IMAGES_PIL = [] # DESC => to stored images with in binary PIL mode.

    try:
        # DESC => checking whether a variable to type.
        if isinstance(files, str):
            # DESC => Single file path
            with open(files, 'rb') as fn:
                with Image.open(fn) as img_pil:
                    temp = img_pil.copy() # DESC => copy the loaded image into variable.
                    IMAGES_PIL.append(temp)
        elif isinstance(files, (list, tuple)):
            # DESC => List of file paths
            for file in files:
            ...
            raise ValueError("Invalid argument type. 'files' must be a string or a list of strings.")
    except FileNotFoundError as error:
        raise FileNotFoundError(f'Error: {error}')
...
@logger.catch()
def measuring_average_color_lightness(r: int = 0, g: int = 0, b: int = 0, factor: float = 0.1, measuring: Optional[str] = None) -> Tuple[int, int, int]:
    ...
    Args:
        ...
        `measuring` (str, optional): The method to measure lightness.
...
# NOTE => Reads an image from the specified file path using OpenCV and returns it as a NumPy array.
@logger.catch()
def read_with_opencv(path: str, astype: Optional[type] = None) -> np.ndarray:
    """Reads an image from the specified file path using OpenCV and returns it as a NumPy array.

    Args:
        `path` (str): The path to the image file.
        `astype` (type, optional): The desired data type of the image. Defaults to None.

    Returns:
        np.ndarray: The image array.

    Raises:
        Exception: If there is an error loading the image using the specified method.
        RuntimeError: If the image is not successfully loaded and is determined to be corrupt.
    ...
    Notes:
        - This function attempts to read the image using two different methods: `imdecode` and `imread`.
        - If the `imdecode` method fails, it falls back to the `imread` method.
        - If both methods fail to load the image, a `RuntimeError` is raised.
        - The `astype` parameter can be used to convert the image array to a specific data type.
    """
    img = None

    try:
        # NOTE => read image as an numpy array
        img_array = np.fromfile(file=path, dtype=np.uint8)
        checker_data = '1-D array' if img_array.ndim == 1 else '2-D array' if img_array.ndim == 2 else '3-D array' if img_array.ndim == 3 else None # <= NOTE check array is 3D & 2D & 1D

        # NOTE => reads an image from a buffer in memory. | input array or vector of bytes.
        img = cv2.imdecode(buf=img_array, flags=cv2.IMREAD_UNCHANGED)
    except Exception as cv_error:
        raise Exception(f'Error loading image, trying with imdecode: {cv_error}')

    if img is None:
        try:
            img = cv2.imread(filename=path, flags=cv2.IMREAD_UNCHANGED)
        except Exception as error:
            raise Exception(f'Error reading image from path "{path}": {error}')

    if img is None:
        raise RuntimeError(f'Error reading image image from path "{path}". Image may be corrupt.')

    if astype is not None:
        img = img.astype(dtype=astype)

    return img # <= NOTE returns image array data

# NOTE => Reads an image from the specified file path using Pillow and returns it as a NumPy array.
@logger.catch()
def read_with_pillow(path: str, astype: Optional[type] = None) -> np.ndarray:
    ...
    Raises:
        Exception: If there is an error reading the image.
    ...
    Notes:
        - This function uses Pillow (Python Imaging Library) to open the image file and convert it into a NumPy array.
        - The `astype` parameter can be used to convert the image array to a specific data type.
    """
    try:
        im = Image.open(fp=path) # <= NOTE open image file with pillow.

        img_array = np.array(im) # <= NOTE convert PIL images into NumPy arrays
        checker_data = '1-D array' if img_array.ndim == 1 else '2-D array' if img_array.ndim == 2 else '3-D array' if img_array.ndim == 3 else None # <= NOTE check array is 3D & 2D & 1D

        if astype is not None:
            img_array = img_array.astype(dtype=astype)

        return img_array
    except Exception as error:
        raise Exception(f'Error reading image from path "{path}": {error}')

# NOTE => Creates a Pillow image object from the specified NumPy array.
@logger.catch()
def read_pillow_from_array(image_array: np.ndarray) -> Image:
    ...
    Raises:
        Exception: If there is an error creating the Pillow Image object
    ...
    """
    try:
        im = Image.fromarray(obj=image_array) # <= NOTE read from array
        return im
    except Exception as error:
        raise Exception(f'Error: {error}')

# NOTE => Converts a Pillow image object to a NumPy array.
@logger.catch()
def pillow_to_numpy(image: Image.Image, astype: Optional[Union[str, np.dtype]] = None) -> np.ndarray:
    """Converts a Pillow image object to a NumPy array.

    Args:
        `image` (Image.Image): The Pillow image object.
        `astype` (Union[str, np.dtype], optional): The desired data type for the NumPy array. Defaults to None.
    ...
    if not isinstance(image, Image.Image):
        raise ValueError(f'{image}: Input must be a PIL.Image.')

    img_array = np.array(image) # <= NOTE convert PIL images into NumPy arrays
    checker_data = '1-D array' if img_array.ndim == 1 else '2-D array' if img_array.ndim == 2 else '3-D array' if img_array.ndim == 3 else None # <= NOTE check array is 3D & 2D & 1D

    if astype is not None:
        img_array = img_array.astype(dtype=astype) # np.uint8
    return img_array

# NOTE => Convert a NumPy array to a PIL.Image.
@logger.catch()
def numpy_to_pillow(img_array: np.ndarray, mode: Optional[str] = None):
    """Convert a NumPy array to a PIL.Image.

    Args:
        `img_array` (np.ndarray): The input NumPy array to be converted.
        `mode` (str, optional): The mode to use when creating the PIL image. Defaults to None.
    ...
    if not isinstance(img_array, np.ndarray):
        raise ValueError(f'{img_array}: Input must be a NumPy array.')
    return Image.fromarray(img_array, mode)

# NOTE => Converts an image from Pillow to OpenCV.
@logger.catch()
def pillow_to_cv2(image_array: np.ndarray) -> np.ndarray:
    """Converts an image from Pillow to OpenCV.

    Args:
        `image_array` (np.ndarray): The image array in Pillow format.

    Returns:
        np.ndarray: The image array in OpenCV format.

    Raises:
        Exception: If there is an error converting the image array.
    ...
    Notes:
        - This function assumes that the input image array is in RGB or RGBA format.
        - It uses the `get_h_w_c_to_image` function to retrieve the height, width, and number of channels of the image array.
        - If the image array has 3 channels, it is converted from RGB color space to BGR color space using `cv2.COLOR_RGB2BGR`.
        - If the image array has 4 channels, it is converted from RGBA color space to BGRA color space using `cv2.COLOR_RGBA2BGRA`.
    """
    try:
        _, _, channels = get_h_w_c_to_image(image_array) # <= NOTE get height width and number of channels. to image

        # NOTE => converts an image from a pillow to OpenCV.
        if channels == 3:
            img_array = cv2.cvtColor(src=img_array, code=cv2.COLOR_RGB2BGR) # <= NOTE converts an image from RGB color space to BGR OpenCV
        elif channels == 4:
            img_array = cv2.cvtColor(src=img_array, code=cv2.COLOR_RGBA2BGRA) # <= NOTE converts an image from RGBA color space to BGRA OpenCV

        return img_array
    except Exception as error:
        raise Exception(f'Error: {error}')

# NOTE => Converts an image from OpenCV to Pillow.
@logger.catch()
def cv2_to_pillow(image_array: np.ndarray) -> np.ndarray:
    """Converts an image from OpenCV to Pillow.

    Args:
        `image_array` (np.ndarray): The image array in OpenCV format.

    Returns:
        np.ndarray: The image array in Pillow format.
    ...
    Notes:
        - This function assumes that the input image array is in BGR or BGRA format.
        - It uses the `get_h_w_c_to_image` function to retrieve the height, width, and number of channels of the image array.
        - If the image array has 3 channels, it is converted from BGR color space to RGB color space using `cv2.COLOR_BGR2RGB`.
        - If the image array has 4 channels, it is converted from BGRA color space to RGBA color space using `cv2.COLOR_BGRA2RGBA`.
    """
    try:
        _, _, channels = get_h_w_c_to_image(image_array) # <= NOTE get height width and number of channels. to image

        # NOTE => converts an image from a OpenCV to pillow.
        if channels == 3:
            img_array = cv2.cvtColor(src=img_array, code=cv2.COLOR_BGR2RGB) # <= NOTE converts an image from BGR OpenCV color space to RGB pillow
        elif channels == 4:
            img_array = cv2.cvtColor(src=img_array, code=cv2.COLOR_BGRA2RGBA) # <= NOTE converts an image from BGRA OpenCV color space to RGBA pillow
...
def time_sleep_accuracy(sleep_time: Union[int, float], measuring_time: Optional[str] = None):
...
def check_is_matching_pattern(sequence_string: str, pattern: Union[str, List], operator: str = "search"):
    ...
    Args:
        ...
        `operator` (str): The check operator the way to check matching 
...
# NOTE => Generate a random seed for NumPy or Python's random module.
def generate_seed(gen_mode: Optional[str] = None, range_low: Optional[int] = None, range_high: Optional[int] = None, seed_fixed: Optional[int] = None, dtype: Optional[str] = None):
    """Generate a random seed for NumPy or Python's random module.

    Args:
        `gen_mode` (str, optional): The mode for generating the seed. If "numpy" it uses NumPy to generate the seed. 
            If "random" it uses Python's random module. Defaults to "numpy"
        `range_low` (int, optional): The lower bound of the random number range of the integer. Defaults to None.
        `range_high` (int, optional): The upper bound of the random number range of the integer. Defaults to None.
        `seed_fixed` (int, optional): A fixed seed value for reproducibility. Defaults to None.
        `dtype` (str, optional): The data type of the generated seed, either "int32" or "int64". Defaults to "int64".

    Returns:
        int: A random seed for use with NumPy or Python's random module.

    Raises:
        ValueError: If the `seed_fixed` is not within the specified data type range or if `range_high` is not within the specified data type range.
    ...
    Notes:
        - If `gen_mode` is not specified, the function defaults to using NumPy if available; otherwise, it uses Python's random module.
        - You can specify the data type of the generated seed, either "int32" or "int64."
        - If seed_fixed is provided, it will be used as the fixed seed, but it will be converted to 32-bit if needed.
        - Setting a custom range is useful when you want random seeds within a specific range.
        - Make sure the specified `range_high` parameters define the valid range on the data type. does not exceed the maximum value representable by a 64-bit integer (2**63-1) or a 32-bit integer (2**31-1).
    """
    generate_low = 0 if range_low is None else range_low
    
    if dtype == "int64":
        generate_high = 2**63 -1 if range_high is None else range_high
        min_bit, max_bit = -(1 << 63), (1 << 63) -1
    elif dtype == "int32":
        generate_high = 2**31 -1 if range_high is None else range_high
        min_bit, max_bit = -(1 << 31), (1 << 31) -1
    else:
        # DESC => for dtype int64.
        generate_high = 2**63 -1 if range_high is None else range_high
        min_bit, max_bit = -(1 << 63), (1 << 63) -1

    if seed_fixed is not None:
        if min_bit <= seed_fixed <= max_bit:
            # DESC =>  convert a 64-bit seed to a 32-bit seed by taking the lower 32 bits of the 64-bit seed
            seed_to_32bit = seed_fixed & 0xFFFFFFFF
            
            # DESC => Set the fixed seed for NumPy operations or set None for is random seed.
            fixed = seed_fixed if dtype == "int32" else seed_to_32bit if dtype == "int64" else None
        else:
            raise ValueError(f"Seed fixed {seed_fixed}: is not a {dtype} bit integer.")
    else:
        fixed = None

    if min_bit <= generate_high <= max_bit:
        if str(gen_mode).lower() == "numpy":
            np.random.seed(fixed)
            random_seed = np.random.randint(low=generate_low, high=generate_high, dtype=np.int64 if dtype == "int64" else np.int32 if dtype == "int32" else np.int64) # DESC => Maximum value of the integer (2^63 - 1 is the largest 64-bit integer)
        elif str(gen_mode).lower() == "random":
            # DESC => Set the fixed seed for Python's random module operations or set None for is random seed.
            random.seed(fixed)
            random_seed = random.randint(a=generate_low, b=generate_high)
        else:
            np.random.seed(fixed)
            random_seed = np.random.randint(low=generate_low, high=generate_high, dtype=np.int64 if dtype == "int64" else np.int32 if dtype == "int32" else np.int64)
        return random_seed
    else:
        raise ValueError(f"Range high {range_high}: is not a {dtype} bit integer.")

# NOTE => Format a size in bytes into KB, MB, GB, or TB.
def format_size_bytes(size_in_bytes: int, unit: Optional[str] = "auto"):
    """Format a size in bytes into KB, MB, GB, or TB.

    Args:
        `size_in_bytes` (float): Size in bytes.
        `unit` (str, optional): The desired unit ('KB', 'MB', 'GB', 'TB', or 'auto' for automatic selection based on size). Defaults to 'auto'

    Returns:
        str: The formatted size with the specified unit, e.g., '5.42 MB'.
    ...
    if str(unit).upper() == 'KB':
        size = size_in_bytes / 1024.0
        return f'{size:.2f} KB'
    elif str(unit).upper() == 'MB':
        size = size_in_bytes / (1024.0 ** 2)
        return f'{size:.2f} MB'
    elif str(unit).upper() == 'GB':
        size = size_in_bytes / (1024.0 ** 3)
        return f'{size:.2f} GB'
    elif str(unit).upper() == 'TB':
        size = size_in_bytes / (1024.0 ** 4)
        return f'{size:.2f} TB'
    elif str(unit).upper() == 'auto':
        if size_in_bytes < 1024:
            return f'{size_in_bytes:.2f} bytes'
        elif size_in_bytes < (1024.0 ** 2):
            size = size_in_bytes / 1024.0
            return f'{size:.2f} KB'
        elif size_in_bytes < (1024.0 ** 3):
            size = size_in_bytes / (1024.0 ** 2)
            return f'{size:.2f} MB'
        elif size_in_bytes < (1024.0 ** 4):
            size = size_in_bytes / (1024.0 ** 3)
            return f'{size:.2f} GB'
        else:
            size = size_in_bytes / (1024.0 ** 4)
...
# NOTE => Check the existence of files at specified paths and information about those files.
def check_paths_exist(folder_root: str, path_files: list[str], total_size: Optional[bool]=None):
    """Check the existence of files at specified paths and information about those files.

    Args:
        `folder_root` (str): The root folder where the files are located.
        `path_files` (list[str]): A list of relative paths to the files within the root folder.
        `total_size` (bool, optional): A list of relative paths to the files within the root folder. Defaults to None.

    Returns:
        Dict[int, Dict[str, Optional[str]]]: A dictionary where keys are path IDs and values are dictionaries
        containing information about each path, including its status (existence), created and modified timestamps,
        and size (if total_size is True).

    Example:
    ```python
        folder_root = r'L:\DATABASE' # Your folder path
        path_files = ['Packages\Test\file0.jpg','Packages\Library\file1.jpg']
    ...
    Notes:
        - The function checks the existence of files at the specified paths within the root folder.
        - It gathers information about creation and modification timestamps.
        - If total_size is True, it calculates and includes the total size of existing files.
    """
    mapping = {}
    total_size_bytes = 0

    for path_id, path_file in enumerate(tqdm(path_files, desc="Check Paths"), start=1):
        full_path = os.path.join(folder_root, path_file)
        if os.path.exists(full_path):
            ...
            if total_size:
                ...
                mapping[path_id] = {
                    "Path": full_path, 
                    "Status": True, 
                    "Size": format_size_bytes(size_in_bytes=total_size_bytes, unit="auto"), 
                    "Created": datetime.datetime.fromtimestamp(created_timestamp).strftime('%Y-%m-%d %H:%M:%S'),
                    "Modified": datetime.datetime.fromtimestamp(modified_timestamp).strftime('%Y-%m-%d %H:%M:%S')
                }
            else:
                mapping[path_id] = {
                    "Path": full_path, 
            ...
            if total_size:
                mapping[path_id] = {"Path": full_path, "Status": False, "Size": None}
            else:
                mapping[path_id] = {"Path": full_path, "Status": False}
    return mapping

# NOTE => Read a CSV file and extract specified columns data.
def reads_csvfile(
    csv_file: str, 
    columns: Optional[Union[list[int], int]] = None, 
    ...
    Args:
        ...
        `columns` (Union[list[int], int], optional): Specifies which columns to extract. 
            Can be an integer for a single column or a list of integers for multiple columns. 
            If None, all columns are extracted. Defaults to None.
        `num_rows` (int, optional):  The maximum number of rows to read. If None, all rows are read. Defaults to None.
        `headers` (bool, optional): Whether the first row is a header row, and columns are extracted based on header names.
            If False, headers are not used, and columns are extracted based on their numeric indices. 
            If None, headers are used if available; otherwise, numeric indices are used. Defaults to None.

    Returns:
        Dict[str, List[str]]: A dictionary where keys are column names (if headers are used) or numeric indices (if headers are not used), 
        and values are lists containing the data from the specified columns.
    ...
    Notes:
        - If headers are not specified, the function will attempt to use the first row as headers if available.
        - You can choose to extract columns either by specifying their numeric indices or by using header names.
        - If `num_rows` is not provided or set to None, all available rows will be read.
        - If `columns` is not provided or set to None, all columns will be extracted.
    """
    column_data = {} # DESC => Dictionary to store data for each column

    # DESC => Open and read the CSV file
    with open(file=csv_file, mode='r', newline='', encoding='utf-8') as csvfile:
        csvreader = csv.reader(csvfile)

        if headers:
            header = next(csvreader) # DESC => Read the header row
            if columns is not None:
                if isinstance(columns, int):
                    if 0 <= columns < len(header):
                        column_data[header[columns]] = []
                elif isinstance(columns, list):
                    for index in columns:
                        if 0 <= index < len(header):
                            column_data[header[index]] = [] # DESC => Use header as key for the column
            else:
                for num_col in range(len(header)):
                    column_data[header[num_col]] = []
        else:
            if columns is not None:
                if isinstance(columns, int):
                    column_data[columns] = []
                ...
                header = next(csvreader, None) # # DESC => Read and count the header row
                for num_col in range(len(header)):
                    column_data[header[num_col]] = []

        row_count = 0 # DESC => Keep track of the number of rows read
        for _row in tqdm(csvreader, desc="Reader CSV"):
            if num_rows is not None and row_count >= num_rows:
                break # DESC => Exit the loop after reading the specified number of rows

            if columns is not None:
                if isinstance(columns, int):
                    if 0 <= columns < len(_row):
                        column_data[header[columns]].append(_row[columns]) if headers else column_data[columns].append(_row[columns])
                elif isinstance(columns, list):
                    for index in columns:
                        if 0 <= index < len(_row):
                            column_data[header[index]].append(_row[index]) if headers else column_data[index].append(_row[index])
            else:
                for index in range(len(header)):
                    column_data[header[index]].append(_row[index]) if headers and columns is None else column_data[header[index]].append(_row[index]) if columns is None else column_data[index].append(_row[index])
            row_count += 1
    return column_data

# NOTE => Writes data to a CSV file.
def writes_csvfile(
    name_csv: Optional[str] = 'data.csv', 
    field_names: Optional[list[str]] = None, 
    dataset: Optional[Union[list[dict], dict]] = None, 
    n_index: Optional[bool] = None):
    """Writes data to a CSV file.

    Args:
        `name_csv` (str, optional): The name of the CSV file to be created. Defaults to 'data.csv'.
        `field_names` (list[str], optional): A list of field names for the CSV columns. 
            Default is None, which auto-generates field names based on the dataset.
        `dataset` (Union[list[dict], dict], optional): The data to be written to the CSV file. 
            If a dictionary is provided, each key represents a row, and its value is a dictionary of 
            column values. If a list of dictionaries is provided, each dictionary represents a row.
            Default is None.
        `n_index` (bool, optional): If True, an 'Index' column is added to the CSV file. Defaults to None.
    ...
    Example:
    ```python
        # DESC => Example 1: Writing a dictionary to CSV
        data_dict = {
            1: {'Row': '100', 'Status': True},
            2: {'Row': '200', 'Status': True},
            3: {'Row': '300', 'Status': True}
        }
        writes_csvfile(name_csv='example.csv', field_names=['Row', 'Status'], dataset=data_dict, n_index=True)

        # DESC => Example 2: Writing a list of dictionaries to CSV
        data_list = [
            {'Row': '100', 'Status': True},
            {'Row': '200', 'Status': True},
            {'Row': '300', 'Status': True}
        ]
        writes_csvfile(name_csv='example.csv', field_names=None, dataset=data_list, n_index=None)
    ```

    Notes:
        - The function creates a new CSV file or overwrites an existing one with the same name.
        - Field names are auto-generated from the dataset if not provided.
        - The 'Index' column is added if `n_index` is set to True.
    """
    if dataset:
        with open(file=name_csv, mode='w', newline='', encoding='utf-8') as csvfile:
            # DESC => Create an ordered dictionary to maintain the column order
            ordered_field_names = OrderedDict()

            # DESC => If field_names is provided, add them in the specified order
            if field_names is not None:
                if n_index:
                    ordered_field_names["Index"] = None
                for field in field_names:
                    ordered_field_names[field] = None
            else:
                # DESC => Determine the field names based on the dataset
                if isinstance(dataset, dict):
                    if n_index:
                        ordered_field_names["Index"] = None
                    for inner_dict in dataset.values():
                        for field in inner_dict.keys():
                            ordered_field_names[field] = None
            ...
            # DESC => Convert the ordered field names to a list
            _field_names = list(ordered_field_names.keys())
            # DESC => Capitalize the first letter of each word in the field names
            _field_names = [name.title() for name in _field_names]
            # DESC => creating a csv dict writer object
            writer = csv.DictWriter(csvfile, fieldnames=_field_names)
            # DESC => writing headers (field names)
            writer.writeheader()

            # DESC => writing dataset rows
            if isinstance(dataset, dict):
                for index, (key, data) in enumerate(tqdm(dataset.items(), desc="Writing CSV"), start=1):
                    if n_index:
                        data.update({"Index": index})
                    writer.writerow(data)
            elif isinstance(dataset, list):
                for index, data in enumerate(tqdm(dataset[:], desc="Writing CSV"), start=1):
                    if n_index:
                        data.update({"Index": index})
                    writer.writerow(data)

# NOTE => Check if a network connection is available.
def check_is_network_available(timeout: Optional[int] = None, logs: Optional[LoggerHandle] = None):
    """Check if a network connection is available.

    Args:
        `timeout` (int, optional): The timeout for the network request in seconds. Default is None.
        `logs` (LoggerHandle, optional): An instance of the LoggerHandle class for logging purposes. Defaults to None.

    Returns:
        bool: True if the network is available, False otherwise.
    """
    try:
        response = requests.get(url="https://www.google.com", timeout=timeout)
        # DESC => Check the status code to ensure a successful response.
        if response.status_code == 200:
            if logs:
                logs.success("Network is available.")
            return True
    except requests.ConnectionError:
        if logs:
            logs.error("Network is not available.")
    return False

# NOTE => Convert a dictionary to a namespace.
def dict_to_namespace(dictionary: dict):
    ...
    Returns:
        SimpleNamespace: A namespace with attributes corresponding to the dictionary keys.

    Example:
    ```python
        my_dict = {'key1': 42, 'key2': 'value'}
        my_namespace = dict_to_namespace(my_dict)
        my_namespace.key1
        42
        my_namespace.key2
        'value'
    ```
    """
    # DESC => convert the dictionary to namespace.
    return SimpleNamespace(**dictionary)

# NOTE => Convert a namespace to a dictionary.
def namespace_to_dict(namespace: SimpleNamespace):
    """Convert a namespace to a dictionary.

    Args:
        `namespace` (SimpleNamespace): The namespace to be converted.

    Returns:
        dict: A dictionary containing the namespace attributes as key-value pairs.

    Example:
    ```python
        my_dict = namespace_to_dict(SimpleNamespace(key1=42, key2='value'))
        my_dict
        {'key1': 42, 'key2': 'value'}

        my_dict = {'key1': 42, 'key2': 'value'}
        my_dict = namespace_to_dict(SimpleNamespace(**my_dict))
        my_dict
        {'key1': 42, 'key2': 'value'}
    ```
    """
    # DESC => convert the namespace to dictionary.
    return dict(namespace.__dict__)

# NOTE => Utility for image processing.
def utils_image( img: Union[np.ndarray, Image.Image, str, bytes], resize_shape: Optional[tuple] = None, convert_color: Optional[str] = None, logs: Optional[LoggerHandle] = None):
    """Utility for image processing.

    Args:
        `img` (Union[np.ndarray, Image.Image, str, bytes]): The input image as a NumPy array, PIL.Image, file path, or image bytes.
        `resize_shape` (tuple, optional): The desired image dimensions as (width, height). Defaults to None.
        `convert_color` (str, optional): The desired color mode for the image ("L", "RGB", or "CMYK"). Defaults to None.
        `logs` (LoggerHandle, optional): An instance of the LoggerHandle class for logging purposes.

    Returns:
        Optional[Image.Image]: The processed image as a PIL.Image object, or None if there was an error.
    """
    try:
        if not isinstance(img, (np.ndarray, Image.Image, str, bytes)):
            if isinstance(logs, LoggerHandle):
                logs.error(f'{img}: Input must be a NumPy array, PIL.Image, string representing an image path, or bytes of an image.')
            return None
        else:
            if isinstance(img, Image.Image):
                if resize_shape:
                    init_image = img.resize(new_shape=resize_shape, resample=Image.Resampling.LANCZOS)
                if convert_color:
                    init_image = img.convert(mode=convert_color)
            elif isinstance(img, np.ndarray):
                convert = numpy_to_pillow(img)
                if resize_shape:
                    init_image = convert.resize(new_shape=resize_shape, resample=Image.Resampling.LANCZOS)
                if convert_color:
                    init_image = convert.convert(mode=convert_color)
            elif isinstance(img, (str, bytes)):
                image_open = Image.open(img)
                if resize_shape:
                    init_image = image_open.resize(new_shape=resize_shape, resample=Image.Resampling.LANCZOS)
                if convert_color:
                    init_image = image_open.convert(mode=convert_color)
        return init_image
    except Exception as error:
        if isinstance(logs, LoggerHandle):
            logs.error(error)
...
# NOTE => Load an image from a file or URL.
def load_image(file_or_url: str, mode_open: Optional[str] = None, mode_operation: Optional[str] = None, outputdir: Optional[str] = None, outputBytesIO: Optional[bool] = None, stream: Optional[bool] = None, logs: Optional[LoggerHandle] = None):
    """Load an image from a file or URL.

    Args:
        `file_or_url` (str): The path to load the image file or URL.
        `mode_open` (str, optional): mode is an optional string that specifies the mode in which the file is opened, "r" reading or "b" binary or "rb" reading binary. Defaults to "r".
        `mode_operation` (str, optional): If choice `"sys"` to open file with python system and `"pil"` operation to open file with Pillow. Defaults to "pil".
        `outputdir` (str, optional): The directory path to save the file image. Defaults to None.
        `outputBytesIO` (bool, optional): Whether to return the image as a BytesIO object. Defaults to None.
        `stream` (bool, optional): Whether to stream the image data. Defaults to None.
        `logs` (LoggerHandle, optional): An instance of the LoggerHandle class for logging purposes.

    Returns:
        Image: Return The Image as PIL object or as BufferedReader or binary object.

    Raises:
        FileNotFoundError: If the image file does not exist.
        URLError: If the image URL cannot be loaded.
    
    Notes:
        - Look the args `outputdir`, `outputBytesIO` and `stream` working if the path image is "URL".
        - `mode_open`: The mode open file `"b"`and `"rb"` not work in `"pil"` operation.
    """
    # DESC => Check if the image path is a URL
    if str(mode_operation).lower() == "sys":
        if file_or_url.startswith('http://') or file_or_url.startswith('https://'):
            # DESC => Load the image from the URL.
            init_image = open(file=requests_get(
                url=file_or_url, 
                outputdir=outputdir, 
                outputBytesIO=outputBytesIO,
                stream=stream,
                logs=logs), mode=mode_open)
        else:
            # DESC => Load the image from the file.
            init_image = open(file=file_or_url, mode=mode_open)
    
    elif str(mode_operation).lower() == "pil" and str(mode_open).lower() == "r":
        if file_or_url.startswith('http://') or file_or_url.startswith('https://'):
            # DESC => Load the image from the URL.
            init_image = Image.open(fp=requests_get(
                url=file_or_url, 
                outputdir=outputdir, 
                outputBytesIO=outputBytesIO,
                stream=stream,
            ...
            # DESC => Load the image from the file.
            init_image = Image.open(fp=file_or_url, mode=mode_open)
    
    elif mode_operation is None:
        if file_or_url.startswith('http://') or file_or_url.startswith('https://'):
            # DESC => Load the image from the URL.
            init_image = Image.open(fp=requests_get(
                url=file_or_url, 
    ...
    return init_image if init_image else None

# NOTE => Convert an image to a BytesIO object.
def image_to_bytesIO(img: Union[Image.Image, np.ndarray, str, bytes]):
    """Convert an image to a BytesIO object.

    Args:
        `img` (Union[Image.Image, np.ndarray, str]): The input image. It can be a PIL Image, a NumPy array,
        or a string representing the image path or bytes.

    Raises:
        TypeError: If the input is not a supported type or if there are other exceptions during the conversion.
    ...
    Example:
    ```python
        # DESC => image to bytesIO of string.
        image_bytes_io = image_to_bytesIO("image.png")

        # DESC => image to bytesIO of pil.
        image_bytes_io = image_to_bytesIO(Image.open("image.png"))

        # DESC => image to bytesIO of array.
        image_bytes_io = image_to_bytesIO(np.ndarray)

        image_bytes_io.getvalue() # Retrieve the byte array of image.
    ```
    """
    try:
        if not isinstance(img, (np.ndarray, Image.Image, str, bytes)):
            raise ValueError(f"{img}: Input must be a NumPy array, PIL.Image, string representing an image path, or bytes of an image.")
        else:
            if isinstance(img, Image.Image):
                # DESC => Create a BytesIO object.
                img_byte_array = BytesIO()

                # DESC => Save the image to the BytesIO object.
                img.save(img_byte_array, format=img.format)
            elif isinstance(img, np.ndarray):
                array_to_pil = Image.fromarray(img)

                img_byte_array = BytesIO()
                array_to_pil.save(img_byte_array, format=array_to_pil.format)
            elif isinstance(img, (str, bytes)):
                img_open = Image.open(img)

                img_byte_array = BytesIO()
                img_open.save(img_byte_array, format=img_open.format)
        return img_byte_array
    except Exception as error:
        raise TypeError(error)

# NOTE => Resizing an image to the closest supported dimension specified in a list.
def resize_image_to_closest_dimension(image: Union[Image.Image, np.ndarray], dimensions: list[tuple[int, int]]):
    """Resizing an image to the closest supported dimension specified in a list.

    Args:
        `image` (Union[Image.Image, np.ndarray]): The input image as a NumPy array or PIL.Image.
        `dimensions` (List[Tuple[int, int]]): A list of supported image dimensions as tuples (width, height).

    Raises:
        ValueError: If the input is not a NumPy array or PIL.Image.

    Returns:
        Image.Image: The processed image resized to the closest supported dimension.

    Example:
    ```python
        # Load an image using PIL or create a NumPy array.
        input_image = Image.open("your_image.jpg")  # or np.array(...)
        supported_dimensions = [(800, 600), (1024, 768), (1920, 1080)]  # Supported dimensions list.

        resized_image = resize_image_to_closest_dimension(input_image, supported_dimensions)
        resized_image.save("output_image.jpg")
    ```

    Notes:
        - This function resizes the input image to the closest supported dimension specified in the dimensions list.
            It uses the minimum sum of absolute differences in width and height to find the closest dimension.
            The resizing is done using Lanczos resampling.
    """
    try:
        if not isinstance(image, (np.ndarray, Image.Image)):
            raise ValueError(f"{image}: Input must be a NumPy array, PIL.Image.")
        else:
            if isinstance(image, Image.Image):
                # DESC => Get the dimensions of the image.
                width, height = image.size

                if (width, height) not in dimensions:
                    # DESC => Find the closest supported dimension.
                    width_closest, height_closest = min(dimensions, key=lambda dim: abs(dim[0] - width) + abs(dim[1] - height))
                    
                    # DESC => Resize the image to the closest supported dimension.
                    img = image.resize((width_closest, height_closest), Image.Resampling.LANCZOS)
                else:
                    # DESC => No need to resize of to closest supported dimension.
                    img = image
            elif isinstance(image, np.ndarray):
                array_to_pil = Image.fromarray(image)

                # DESC => Get the dimensions of the image.
                width, height = array_to_pil.size
                ...
                if (width, height) not in dimensions:
                    ...
                    # DESC => Resize the image to the closest supported dimension.
                    img = array_to_pil.resize((width_closest, height_closest), Image.Resampling.LANCZOS)
                else:
                    # DESC => No need to resize of to closest supported dimension.
                    img = image
        return img
...
# NOTE => Resize an image to the closest supported dimensions of pixels that exceed a maximum limit.
def resize_image_to_closest_max_pixels(image: Union[Image.Image, np.ndarray], max_total_pixels: int):
    """Resize an image to the closest supported dimensions of pixels that exceed a maximum limit.

    Args:
        `image` (Union[Image.Image, np.ndarray]): The input image as a NumPy array or PIL.Image.
        `max_total_pixels` (int): The maximum total pixels allowed. Must be a positive number.

    Raises:
        ValueError: If the input is not a NumPy array or PIL.Image.
        ValueError: If max_total_pixels is not a positive number.

    Returns:
        Image.Image: The resized image as a PIL.Image.
    
    Example:
    ```python
        # Load an image using PIL or create a NumPy array.
        input_image = Image.open("your_image.jpg") # or np.array(...)
        max_pixels = 1048576 # Maximum total pixels.

        resized_image = resize_image_to_closest_max_pixels(input_image, max_pixels)
        resized_image.save("output_image.jpg")
    ```

    Notes:
        - This function resizes the input image to fit within the specified maximum total pixels
    ...
    try:
        if not isinstance(image, (np.ndarray, Image.Image)):
            ...
            if max_total_pixels <= 0:
                raise ValueError("max_total_pixels must be a positive number.")

            if isinstance(image, Image.Image):
                # DESC => Get the dimensions of the image and calculate total pixels.
                width, height, current_total_pixels = (image.size[0], image.size[1], image.size[0]*image.size[1])

                if current_total_pixels <= max_total_pixels:
                    # DESC => Image is within the limit, no need to resize.
                    img = image
                else:
                    # DESC => Calculate the new dimensions to fit within the limit.
                    new_width, new_height = (int((max_total_pixels / current_total_pixels) ** 0.5 * width), 
                                            int((max_total_pixels / current_total_pixels) ** 0.5 * height))

                    # DESC => Resize the image to the new dimensions.
                    img = image.resize((new_width, new_height), Image.Resampling.LANCZOS)
            elif isinstance(image, np.ndarray):
                array_to_pil = Image.fromarray(image)

                # DESC => Get the dimensions of the image and calculate total pixels.
                width, height, current_total_pixels = (array_to_pil.size[0], array_to_pil.size[1], array_to_pil.size[0]*array_to_pil.size[1])
                ...
                if current_total_pixels <= max_total_pixels:
                    ...
                    # DESC => Resize the image to the new dimensions.
                    img = array_to_pil.resize((new_width, new_height), Image.Resampling.LANCZOS)
        return img
    except Exception as error:
        raise error

# NOTE => Resize an shape to the closest supported dimensions and a maximum pixels limit.
def closest_supported_dimensions_and_max_pixels(shape: tuple[int, int], max_total_pixels: Optional[int] = None, supported_dimensions: Optional[list[tuple[int, int]]] = None):
    """Resize an shape to the closest supported dimensions and a maximum pixels limit.

    Args:
        `shape` (tuple[int, int]): The original shape as a tuple of (width, height).
        `max_total_pixels` (int, opstion): The maximum total pixels allowed. Must be a positive number.
        `supported_dimensions` (list[tuple[int, int]], optional): List of supported dimensions as tuples (width, height).

    Raises:
        ValueError: If max_total_pixels is not a positive number.

    Returns:
        tuple[int, int]: The closest supported dimensions for the input shape.
    
    Example:
    ```python
        shape = (800, 600)
        max_total_pixels = 1048576
        supported_dimensions = [(800, 600), (1024, 768), (1280, 720)]

        result = closest_supported_dimensions_and_max_pixels(shape, max_total_pixels, supported_dimensions)
        # Output: (1024, 768)
    ```

    Notes:
        - If both max_total_pixels and supported_dimensions are provided, the function will first check
            if the shape is within the limit. If it is, no resizing is needed. If it exceeds the limit,
            it will calculate the new dimensions to fit within the limit and then find the closest
            supported dimension.
        - If only max_total_pixels is provided, it will check if the shape is within the limit and resize if necessary.
        - If only supported_dimensions are provided, it will find the closest supported dimension.
        - If neither max_total_pixels nor supported_dimensions are provided, it returns the original shape.
    """
    try:
        if max_total_pixels and supported_dimensions:
            if max_total_pixels <= 0:
                raise ValueError("max_total_pixels must be a positive number.")

            # DESC => Get the dimensions and calculate total pixels.
            width, height, current_total_pixels = (shape[0], shape[1], shape[0]*shape[1])

            if current_total_pixels <= max_total_pixels:
                # DESC => shape is within the limit, no need to resize.
                _shape = shape
            else:
                # DESC => Calculate the new dimensions to fit within the limit.
                new_width, new_height = (int((max_total_pixels / current_total_pixels) ** 0.5 * width), 
                ...
                # DESC => Resize the shape to the new dimensions.
                _shape = (new_width, new_height)

            if (_shape[0], _shape[1]) not in supported_dimensions:
                # DESC => Find the closest supported dimension.
                width_closest, height_closest = min(supported_dimensions, key=lambda dim: abs(dim[0] - _shape[0]) + abs(dim[1] - _shape[1]))
                # DESC => Resize the shape to the new dimensions.
                _shape = (width_closest, height_closest)
        elif max_total_pixels:
            if max_total_pixels <= 0:
                raise ValueError("max_total_pixels must be a positive number.")
            ...
            if (width, height) not in supported_dimensions:
                # DESC => Find the closest supported dimension.
                width_closest, height_closest = min(supported_dimensions, key=lambda dim: abs(dim[0] - width) + abs(dim[1] - height))
                # DESC => Resize the shape to the new dimensions.
                _shape = (width_closest, height_closest)
            else:
                # DESC => No need to resize of to closest supported dimension.
...
class Image_ColorAverageDominant:
    """Extract color average and dominant"""
    def __init__(self, logs: Optional[LoggerHandle]=None): # DESC => initialize constructor
        """Initialize the Image_ColorAverageDominant object.
    ...
    def extract_average_colore(self, image: str):
        """
        ...
        Returns:
            tuple[int, int, int]: The RGB values of the average color.
        ...
        Example:
        ```python
            image_processor = Image_ColorAverageDominant(logs=True)
            # Extract average color
            image_path = "image.jpg"
            average_color = image_processor.extract_average_color(image_path)
            print("Average Color RGB:", average_color) # output (12, 55, 32)
        ```
        """
        try:
            ...
            avg_color_per_row = np.average(src, axis=0)
            avg_color = np.average(avg_color_per_row, axis=0)

            (blue, green, red) = (avg_color[0], avg_color[1], avg_color[2]) # DESC => It returns BGR [blue, green, red]
            RGB = (int(round(red[0], 0)), int(round(green[0], 0)), int(round(blue[0], 0)))
            
            self.logs.info(f'[INFO AVERAGE]', f'RGB(red={RGB[0]}, green={RGB[1]}, blue={RGB[2]})') if isinstance(self.logs, LoggerHandle) else None
            return RGB
        except Exception as error:
            raise Exception(f'Error: {error}')

    def extract_dominant_colore(self, image: str, n_dominant: int=5, show_visualize: bool=False):
        """
        Extract the dominant colors from an image.

        Args:
            `image` (str): The path to the image.
            `n_dominant` (int, optional): The number of dominant colors to extract. Defaults to `5`.
            `show_visualize` (bool, optional): Specifies whether to visualize the dominant colors. Defaults to `False`.

        Returns:
            `dict`: A dictionary containing information about the dominant colors.
        ...
        ```python
            ...
            image_path = "image.jpg"
            # Number Find and display most dominant colors.
            n_dominant = 2 
            dominant_colors = image_processor.extract_dominant_colore(image_path, n_dominant=n_dominant, show_visualize=False)
            print(dominant_colors) 
            # output 
            # color 1: {'percent': '23.4%', 'RGB': {'red': 12, 'green': 33, 'blue': 32}}
            # color 2: {'percent': '34.4%', 'RGB': {'red': 1, 'green': 223, 'blue': 132}}
        ```
        """
        try:
            # DESC => load and read image with opencv
            ...
            img = cv2.cvtColor(src=src, code=cv2.COLOR_BGR2RGB)

            reshape = img.reshape((img.shape[0] * img.shape[1], 3))

            # DESC => Find and display most dominant colors
            cluster = KMeans(n_clusters=n_dominant, n_init='auto').fit(reshape)
            centroids = cluster.cluster_centers_

            # DESC => Get the number of different clusters, create histogram, and normalize
            labels = np.arange(0, len(np.unique(cluster.labels_)) + 1)
            (hist, _) = np.histogram(cluster.labels_, bins = labels)
            hist = hist.astype("float")
            hist /= hist.sum()

            # DESC => Create frequency rect and iterate through each cluster's color and percentage
            colors_palette = np.zeros((100, 300, 3), dtype=np.uint8)
            colors = sorted([(percent, color) for (percent, color) in zip(hist, centroids)])

            start = 0
            dict_colors_palette = {}

            for index, (percent, color) in enumerate(colors):
                self.logs.info(f"""
                [INFO COLOR-{int(index +1)}] RGB(red={int(round(color[0], 0))}, green={int(round(color[1], 0))}, blue={int(round(color[2], 0))}) {round(float(percent * 100), 2)}%
                """) if isinstance(self.logs, LoggerHandle) else None

                dict_colors_palette[f'color[{int(index +1)}]'] = {'percent': f'{round(float(percent * 100), 2)}%', 'RGB': {'red': int(round(color[0], 0)), 'green': int(round(color[1], 0)), 'blue': int(round(color[2], 0))}}

                end = start + (percent * 300)
                cv2.rectangle(img=colors_palette, pt1=(int(start), 0), pt2=(int(end), 100), \
                            color=color.astype(np.uint8).tolist(), thickness=-1)
                start = end

            # DESC => display it to our screen
            if show_visualize:
                # DESC => converts color space conversion code
                visualize = cv2.cvtColor(src=colors_palette, code=cv2.COLOR_RGB2BGR)

                cv2.imshow(winname='Image', mat=visualize)
                if cv2.waitKey(0) == ord('q'):
                    cv2.destroyAllWindows()

            return dict_colors_palette
        except Exception as error:
            raise Exception(f'Error: {error}')

class AutoCorrection_WhiteBalance:
    """Auto white balance correction opencv."""
    def __init__(self, logs: Optional[LoggerHandle]=None): # DESC => initialize constructor
        """Initialize AutoCorrection_WhiteBalance the object.
    ...
    def gray_world(self, nimg: np.ndarray):
        """Apply gray world algorithm for white balance correction
    ...
    def max_white(self, nimg: np.ndarray):
        """Apply max white algorithm for white balance correction

        Args:
            `nimg` (np.ndarray): _description_
    ...
    def retinex(self, nimg: np.ndarray):
        """Apply retinex algorithm for white balance correction
    ...
    def retinex_adjust(self, nimg: np.ndarray):
        """Apply retinex adjustment after retinex algorithm for white balance correction
    ...
    def retinex_with_adjust(self, nimg: np.ndarray):
        """Apply retinex algorithm followed by retinex adjustment for white balance correction
    ...
    def gimp(self, image: str, type_correction: Literal['world', 'max_white', 'retinex', 'retinex_adjust', 'retinex_with_adjust']='world', perc: float=0.05, save: str=None, show_visualize: bool=False, combining: bool=True):
        """Perform white balance correction on an image using various algorithms.

        Args:
            `image` (str): The path to the input image file.
            `type_correction` (Literal): ['world', 'max_white', 'retinex', 'retinex_adjust', 'retinex_with_adjust']
                The type of white balance correction algorithm to use. Default is 'world'.
            `perc` (float, optional): The percentile value for contrast enhancement. Default is 0.05.
            `save` (str, optional): The path to save the output image. If not provided, the output will not be saved. Defaults to None.
            `show_visualize` (bool, optional): Whether to display the visualized output image. Defaults to False.
            `combining` (bool, optional): Whether to combine the corrected image with the original image for visualization. Defaults to True.

        Returns:
            None

        Raises:
            ValueError: If an invalid type_correction value is provided.

        Example:
        ```python
            auto_correction = AutoCorrection_WhiteBalance()
            auto_correction.gimp(image='input.jpg', type_correction='world', save='output.jpg', show_visualize=False)
        ```
        """
        # DESC => load and read image with opencv
        src = cv2.imread(filename=image)

        if type_correction == 'world':
            img = self.gray_world(nimg=src)
        elif type_correction == 'max_white':
            img = self.max_white(nimg=src)
        elif type_correction == 'retinex':
            img = self.retinex(nimg=src)
        elif type_correction == 'retinex_adjust':
            img = self.retinex_adjust(nimg=src)
        elif type_correction == 'retinex_with_adjust':
            img = self.retinex_with_adjust(nimg=src)
        else:
            raise ValueError("Please choice type_correction: ['world', 'max_white', 'retinex', 'retinex_adjust', 'retinex_with_adjust']")
        ...
        try:
            ...
            if save is not None:
                if save == '':
                    # DESC => split file path
                    basename, extension = os.path.splitext(os.path.basename(image))
                    dirname, basename = os.path.split(os.path.splitext(image)[0])
                else:
                    # DESC => split file path
                    basename, extension = os.path.splitext(os.path.basename(save))
                    dirname, basename = os.path.split(os.path.splitext(save)[0])

                output_path = os.path.join(dirname, f'{basename}-res{extension}')

                # DESC => write the generated ArUCo tag to disk
                cv2.imwrite(filename=output_path, img=img)
                self.logs.info(f"Output path saved: {output_path}") if isinstance(self.logs, LoggerHandle) else None

            # DESC => display it to our screen
            if show_visualize:
                if combining:
                    # DESC => draw text string
                    #/ img = cv2.putText(img=img, text='source old', org=(-1, -1), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=8, color=(255, 0, 0), thickness=2)

                    # DESC => setup visualize
                    h_vis = np.hstack((img, src)) # <= DESC => order to stack horizontally
                    v_vis = np.vstack((img, src)) # <= DESC => order to stack vertically

                    # DESC => resize the reference and input images
                    img = imutils.resize(image=h_vis, width=1000)
                    cv2.imshow(winname='Combining Image', mat=img)
                else:
                    cv2.imshow(winname='Gimp Image', mat=img)
...
class AutoColorMatch:
    """Automated color matching using histogram matching."""
    def __init__(self, logs: Optional[LoggerHandle]=None): # DESC => initialize constructor
        """Initialize the AutoColorMatch object.
        Args:
            `logs` (Optional[LoggerHandle], optional): An instance of the LoggerHandle class for logging purposes.
    ...
    def color_match(self, image: str, reference: str, save: str=None, show_visualize: bool=False, compare: bool=False) -> None:
        """Perform color matching on an image using histogram matching.

        Args:
            `image` (str): Path to the input image.
            `reference` (str): Path to the reference image for color matching.
            `save` (str, optional): Path to save the matched image. Defaults to None.
            `show_visualize` (bool, optional): Flag indicating whether to display the matched image. Defaults to False.
            `compare` (bool, optional): Flag indicating whether to compare the original and matched images. Defaults to False.

        Returns:
            None

        Raises:
            Exception: If an error occurs during the color matching process.

        Example:
        ```python
            auto_match = AutoColorMatch()
            auto_match.color_match(image='input.jpg', reference='reference.jpg', save='output.jpg', show_visualize=False, compare=True)
        ```

        Notes:
            - Make sure to provide valid paths to the input and reference images.
            - The input image will be modified in-place with the matched color.
            - If `save` is specified, the matched image will be saved to the specified path.
            - If `show_visualize` is True, the matched image will be displayed on the screen.
            - If `compare` is True, both the original and matched images will be displayed for comparison.
        """
        try:
            # DESC => load and read image with skimage
            ref = io.imread(fname=reference, as_gray=False)
            img = io.imread(fname=image, as_gray=False)

            # DESC => Match using the right side as reference
            matched = exposure.match_histograms(image=img, reference=ref, channel_axis=-1)
            ...
            if save is not None:
                ...
                output_path = os.path.join(dirname, f'{basename}-match{extension}')

                # DESC => write the generated ArUCo tag to disk
                io.imsave(fname=output_path, arr=matched, check_contrast=True)
                self.logs.info(f"Output path saved: {output_path}") if isinstance(self.logs, LoggerHandle) else None
            ...
            if show_visualize:
                # DESC => setup visualize
                if compare:
                    self.matplotlib_visualize(image_one=img, image_two=matched, label_1='old', label_2='new', compare=True)
                else:
                    self.matplotlib_visualize(image_one=matched, label_1='new', compare=False)
        except Exception as error:
            raise Exception(f'Error: {error}')

    def color_match_transfer(self, target: str, source: str, clip: bool=True, preserve_paper: bool=True, save: str=None, show_visualize: bool=False, compare: bool=False) -> None:
        ...
        Args:
            ...
            `source` (str): Path to the source image for color transfer.
            `clip` (bool, optional): Flag indicating whether to scale the L*a*b* image components by np.clip. Defaults to True.
            `preserve_paper` (bool, optional): Flag indicating whether to strictly follow the methodology laid out in the original paper. Defaults to True.
            `save` (str, optional): Path to save the transferred image. Defaults to None.
            `show_visualize` (bool, optional): Flag indicating whether to display the transferred image. Defaults to False.
            `compare` (bool, optional): Flag indicating whether to compare the source and transferred images. Defaults to False.

        Returns:
            None

        Raises:
            Exception: If an error occurs during the color transfer process.

        Example:
        ```python
            # Load and transfer color from source to target image
            transfer = ColorTransfer()
            transfer.color_match_transfer(target='target.jpg', source='source.jpg', save='output.jpg', show_visualize=True)
        ```
        ...
        Notes:
            ...
            - This method uses the mean and standard deviations of the L*a*b* color space to transfer color distribution.
            - The `save` parameter specifies the path to save the transferred image. If not provided, the image will not be saved.
            - The `show_visualize` parameter controls whether to display the transferred image.
            - The `compare` parameter can be used to compare the source and transferred images side by side.
        """
        def image_stats(nimg: np.ndarray):
            """Compute the mean and standard deviations for the L*, a*, and b* channels of an L*a*b* image.

            Args:
                `nimg` (np.ndarray): NumPy array representing an L*a*b* image.

            Returns:
                Tuple of mean and standard deviations for the L*, a*, and b* channels, respectively.
            """
            # DESC => compute the mean and standard deviation of each channel
            (l, a, b) = cv2.split(nimg)
            (lMean, lStd) = (l.mean(), l.std())
            (aMean, aStd) = (a.mean(), a.std())
            (bMean, bStd) = (b.mean(), b.std())

            # DESC => return the color statistics
            return (lMean, lStd, aMean, aStd, bMean, bStd)

        def min_max_scale(arr: np.ndarray, new_range=(0, 255)):
            """Perform min-max scaling to a NumPy array.

            Args:
                `arr` (np.ndarray): NumPy array to be scaled to [new_min, new_max] range.
                `new_range` (tuple, optional): Tuple of form (min, max) specifying range of transformed array. Defaults to (0, 255).

            Returns:
                NumPy array that has been scaled to be in [new_range[0], new_range[1]] range.
            """
            # DESC => get array's current min and max
            mn = arr.min()
            mx = arr.max()

            # DESC => check if scaling needs to be done to be in new_range
            if mn < new_range[0] or mx > new_range[1]:
                # DESC => perform min-max scaling
                scaled = (new_range[1] - new_range[0]) * (arr - mn) / (mx - mn) + new_range[0]
            else:
                # DESC => return array if already in range
                scaled = arr
            return scaled

        def scale_array(arr: np.ndarray, clip=True):
            """Trim NumPy array values to be in [0, 255] range with option of clipping or scaling.

            Args:
                `arr` (np.ndarray): Array to be trimmed to [0, 255] range.
                `clip` (bool, optional): Flag indicating whether to scale the array by np.clip. Defaults to True.
        ...
        try:
            # DESC => load and read image with opencv
            src = cv2.imread(filename=source)
            tar = cv2.imread(filename=target)

            # DESC => convert the images from the RGB to L*ab* color space, being sure to utilizing the floating point data type
            # DESC => (note: OpenCV expects floats to be 32-bit, so use that instead of 64-bit)
            src = cv2.cvtColor(src=src, code=cv2.COLOR_BGR2LAB).astype(dtype=np.float32)
            tar = cv2.cvtColor(src=tar, code=cv2.COLOR_BGR2LAB).astype(dtype=np.float32)

            # DESC => compute color statistics for the source and target images
            (lMeanSrc, lStdSrc, aMeanSrc, aStdSrc, bMeanSrc, bStdSrc) = image_stats(src)
            (lMeanTar, lStdTar, aMeanTar, aStdTar, bMeanTar, bStdTar) = image_stats(tar)

            # DESC => subtract the means from the target image
            (l, a, b) = cv2.split(tar)
            ...
            if preserve_paper:
                # DESC => scale by the standard deviations using paper proposed factor
                l = (lStdTar / lStdSrc) * l
                a = (aStdTar / aStdSrc) * a
                b = (bStdTar / bStdSrc) * b
            else:
                # DESC => scale by the standard deviations using reciprocal of paper proposed factor
                l = (lStdSrc / lStdTar) * l
                a = (aStdSrc / aStdTar) * a
                b = (bStdSrc / bStdTar) * b

            # DESC => add in the source mean
            l += lMeanSrc
            a += aMeanSrc
            b += bMeanSrc

            # DESC => clip/scale the pixel intensities to [0, 255] if they fall outside this range
            l = scale_array(l, clip=clip)
            a = scale_array(a, clip=clip)
            b = scale_array(b, clip=clip)

            # DESC => merge the channels together and convert back to the RGB color space, being sure to utilize the 8-bit unsigned integer data type
            transfer = cv2.merge([l, a, b])
            transfer = cv2.cvtColor(src=transfer.astype(dtype=np.uint8), code=cv2.COLOR_LAB2BGR)
        except Exception as error:
            raise Exception(f'Error: {error}')
        finally:
            # DESC => write the generated ArUCo tag to disk
            if save is not None:
                if save == '':
                    # DESC => split file path
                    basename, extension = os.path.splitext(os.path.basename(target))
                ...
                output_path = os.path.join(dirname, f'{basename}-transfer{extension}')

                # DESC => write the generated ArUCo tag to disk
                cv2.imwrite(filename=output_path, img=transfer)
                self.logs.info(f"Output path saved: {output_path}") if isinstance(self.logs, LoggerHandle) else None
            ...
            if show_visualize:
                if compare:
                    # DESC => setup visualize
                    self.opencv_visualize(image=cv2.imread(filename=source), label='source', width=300)
                    self.opencv_visualize(image=transfer, label='target', width=300)
                else:
                    self.opencv_visualize(image=transfer, label='target', width=300)
    ...
    def opencv_visualize(self, image: np.ndarray, label: str='image', width: int=300):
        """Displays an image using OpenCV.

        Args:
            `image` (np.ndarray): Image as a NumPy array.
            `label` (str, optional): Label for the window. Defaults to 'image'.
            `width` (int, optional): Width for resizing. Defaults to 300.

        Returns:
            None
        """
        # DESC => resize the image to have a constant width, just to make displaying the images
        resized = self.resize_constant(nimg=image, width=width)

        # DESC => show the resized image
        cv2.imshow(winname=label, mat=resized)

    def matplotlib_visualize(self, image_one: np.ndarray, image_two: np.ndarray, label_1: str='image_one', label_2: str='image_two', width: int=300, compare: bool=False):
        """Displays two images side by side using Matplotlib.

        Args:
            `image_one` (np.ndarray): First image as a NumPy array.
            `image_two` (np.ndarray): Second image as a NumPy array.
            `label_1` (str, optional): Label for the first image. Defaults to 'image_one'.
            `label_2` (str, optional): Label for the second image. Defaults to 'image_two'.
            `width` (int, optional): Width for resizing. Defaults to 300.
            `compare` (bool, optional): Flag to compare the images. Defaults to False.

        Returns:
            None
        """
        # DESC => resize the image to have a constant width, just to make displaying the images
        resized_one = self.resize_constant(nimg=image_one, width=width)
        resized_two = self.resize_constant(nimg=image_two, width=width)

        if compare:
            # DESC => Create just a figure and only one subplot
            fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2)

            ax1.imshow(resized_one, cmap=None)
            ax1.set_axis_on()
            ax1.set_title(label=label_1)

            ax2.imshow(resized_two, cmap=None)
            ax2.set_axis_on()
            ax2.set_title(label=label_2)
        ...
        plt.tight_layout()
        plt.show() # DESC => attached below

    def resize_constant(self, nimg: np.ndarray, width: int):
        """Resizes an image to a constant width while maintaining the aspect ratio.

        Args:
            `nimg` (np.ndarray): Image as a NumPy array.
            `width` (int): Desired width for resizing.

        Returns:
            np.ndarray: Resized image as a NumPy array.
        """
        resize = width / float(nimg.shape[1])
        dim = (width, int(nimg.shape[0] * resize))
